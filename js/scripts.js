            ...
            if (data.length > 0) {
              ...
              if (userInfo.description != previousReleaseDescription) {
                ...
                if (newVersion != '') {
                    ...
                    success: function (data) {
                      ...
                      $.ajax({
                        ...
                        error: function (jqXHR, textStatus, errorThrown) {
                          ...
                          $('#successMsg').html("");
                          $('#errorMsg').html("An unknown error occurred while creaing the release. Please try again.");
                        }
                      });
                    },
                  ...
                  $('.msg-container').css({ "background-color": "#F21905" });
                  $('#successMsg').html("");
                  $('#errorMsg').html("Unable to read current format of version number in library.properties file or the version number of the release tag in releases. Please change library.properties version to be in the format of '#.#.#' (i.e. 'version = 1.0.0'). This program currently supports versions up to 100.100.100.");
                }
                // CASE 2d: Check if release created accidently
              } else {
                $('.msg-container').css({ "background-color": "#F21905" });
                $('#successMsg').html("");
                $('#errorMsg').html("The release description is the same as the previous release description. To ensure this is not a duplicate release, please update the description before trying again.");
              }
              ...
              $('.msg-container').css({ "background-color": "#F21905" });
              $('#successMsg').html("");
              $('#errorMsg').html("No release yet. Please create the first release yourself. Don't forget to create an issue on the Arduino GitHub to ensure it's included in the Arduino Library Manager.");
            }
          }
        });
      },
